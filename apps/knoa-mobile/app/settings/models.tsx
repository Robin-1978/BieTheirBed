    ...
    try {
      const draft = await gateway.runAuthenticated(async (client) => {
        ...
        document.providers[providerId.trim()] = {
          ...
          api_base: driver === "llamacpp" ? "" : endpoint.trim(),
          api_key_ref: driver === "llamacpp" ? "" : secretRef.trim(),
          api_key_env: "",
          secret_version: nextSecretVersion
            ? Math.max(nextSecretVersion, (existing?.secret_version ?? 0) + 1)
            : existing?.secret_version ?? 0,
          requires_api_key: driver === "llamacpp" ? false : true,
          timeout_seconds: 120,
        };
    ...
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.section}>
        ...
        <View style={styles.choices}>
          {(["openai_compatible", "openai", "anthropic", "llamacpp"] as Driver[]).map((value) => (
        ...
        <Field value={endpoint} onChange={setEndpoint} placeholder={driver === "llamacpp" ? "http://127.0.0.1:8080" : "API Base URL"} />
        {driver !== "llamacpp" ? <><Field value={secretRef} onChange={setSecretRef} placeholder="Secret reference" /><Field value={secret} onChange={setSecret} placeholder="API Key（写入后清空）" secure /></> : null}
