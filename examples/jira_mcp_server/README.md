...
`mcp.yaml`. Deploy or update the same stable server ID with:

```bash
scripts/knoa mcp-package-deploy /absolute/path/to/examples/jira_mcp_server jira
```
...
package. User automation belongs to a Knoa Task Definition and is not embedded
...
Automatic issue analysis needs one user-owned Task Definition. The MCP Server
exposes `jira://assigned-to-me/events` as the stable event collection; configure
the Task to match descendants only. The MCP Server does not create or control
Knoa Tasks.

## 3. Create the event Task

Use the App, CLI or Textual TUI. For example:

```bash
scripts/knoa task-create-event jira jira://assigned-to-me/events \
  "Analyze newly assigned Jira issues" \
  --title "Analyze Jira assignments" \
  --agent-id codex \
  --descendants-only
```

`JIRA_JQL` remains private MCP Server configuration. The Task stores only the
generic Resource selector and does not contain Jira query logic.
...
idempotent Execution under the configured durable Task per Resource event. The instruction contains the absolute
