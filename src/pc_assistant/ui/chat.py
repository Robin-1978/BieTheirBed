...
class ChatUI:
    ...
    async def _handle_user_command(self, command: str) -> bool:
        ...
        if cmd in ("/exit", "/quit"):
            ...
            table.add_row("Provider", model.provider_name)
            table.add_row("Driver", model.driver)
            table.add_row("Server", model.server_url)
            table.add_row("Model", model.alias)
            table.add_row("Upstream Model", model.model or "(not set)")
            table.add_row("Configured Models", ", ".join(self._config.models) or model.alias)
            table.add_row("API Key", "configured" if model.api_key else "")
