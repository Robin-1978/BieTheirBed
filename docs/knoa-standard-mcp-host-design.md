...
>
> 范围：Knoa 作为标准 MCP Host，PC Assistant MCP 与 Jira MCP 作为平级能力服务；Jira 自动问题分析作为首个真实纵向示例；Native Agent 与 Codex App Server 共享同一产品和能力边界。
...
Knoa Core
  ...
  `- Standard MCP Host
       |- PC Assistant MCP Server
       |- Jira MCP Server
       `- other standard MCP servers
```

PC Assistant MCP 负责本地设备和工作区能力，Jira MCP 负责 Jira 业务能力。二者在 MCP capability layer 平级，彼此不依赖。Knoa Core 不包含 Jira 类型判断，也不把外部 MCP Prompt 提升为系统策略。
...
- Agent 生命周期聚合在 AgentRuntimePort/Provider。
- Task、Approval、Artifact 和 Conversation 继续由 Knoa Core 唯一拥有。
- Jira API、JQL、评论、附件、写回和轮询聚合在 Jira MCP Server。
- 本地文件、代码、Shell、桌面和视觉能力逐步聚合到 PC Assistant MCP Server。

### 2.3 低耦合

- Knoa Core 不出现 Jira、GitLab、Monitor 等业务分支。
- Jira MCP 不依赖 PC Assistant MCP。
- PC Assistant MCP 不理解 Jira。
...
Conversation 是用户和 Knoa Agent 的持续交互面，不属于 PC Assistant MCP 或 Jira MCP。
...
Agent 根据任务选择 Jira MCP、PC Assistant MCP 或二者组合。
...
## 8. Native Agent 与 Codex App Server

### 8.1 运行时无关的事件入口

MCP Resource Task Source 先创建 Knoa Task，再由 `AgentRuntimePort` 选择 Provider：
...
Jira MCP 和 PC Assistant MCP 不知道执行 Provider。

### 8.2 Native Agent

Native Agent 通过 Knoa Capability Registry 使用 built-in tools 和 MCP tools。第一期 Resource Task Bridge 复用现有 ToolStep、Approval 和 Artifact 边界。

### 8.3 后续 Codex Provider Change：App Server 适配

Codex App Server 是 `AgentRuntimeProvider`，不是 MCP Tool，也不是 Knoa 的 MCP Host 替代品。官方协议面向富客户端，负责 Codex 上游账户认证、Thread、Turn、Item、流式事件、审批和中断；它使用省略 `jsonrpc` 字段的双向 JSON-RPC 2.0。Codex 账户认证不提供 Knoa principal 隔离，principal、Session、workspace ownership 始终由 Knoa Core 持有和校验。App 始终只连接 Knoa 产品 API。

后续 Codex Provider MVP 使用本机 `stdio` JSONL：
...
Knoa 持久化 `session_handle -> provider_thread_id` 和 `turn_id -> provider_turn_id` 绑定；所有 App Server 主动请求必须先通过 Thread binding 反查 principal、Session 和 workspace，不能仅凭 Provider ID 路由。App Server 断线后可用 `thread/read`/`thread/resume` 与 Knoa Snapshot 进行已持久化历史 reconciliation，但这不恢复旧连接上的待决 JSON-RPC server request，也不证明未确认外部写操作的结果。

生产 Provider 必须使用隔离、生成式 Codex 配置目录，不能继承用户已有的 MCP、apps 或 plugins 能力。若后续启用 Capability Gateway，启动后还必须通过 `mcpServerStatus/list` 核验 inventory 只包含当前 session-scoped Gateway；发现任何额外外部能力即拒绝进入 READY。App Server 进程和 Codex 凭据至少不得跨不可信 Knoa principal 共享。具体 Provider 数据模型、事件映射和 supervisor 约束见 [Knoa Agent Runtime Provider 设计](knoa-agent-runtime-provider-design.md)。

Provider 必须使用当前安装的 Codex 版本生成的 schema，而不是手写长期固定字段：

```bash
codex app-server generate-json-schema --out <managed-schema-dir>
```

生成 schema 是 Provider 私有兼容资料，不进入 Knoa 公共 API。升级 Codex 前必须通过协议契约测试验证 initialize、thread/turn、通知、审批和中断映射。

官方参考：[Codex App Server](https://developers.openai.com/codex/app-server/)。

### 8.4 后续 Codex Provider Change：能力接入

Codex 不得直接连接任何未经 Knoa policy boundary 的外部 Jira/PC Assistant MCP，不以工具当前是否只读或高风险作为例外，因为风险分类和 Server 能力都会变化。
...
接入候选分两步，但都属于后续独立 Change：

1. MVP：在 `thread/start` 注册当前 Session 允许的 App Server `dynamicTools`；Codex 通过 `item/tool/call` 把调用回送 `CodexAppServerProvider`，再进入 Knoa Capability Registry；
2. Target：Knoa 暴露 session-scoped 标准 MCP Capability Gateway；Codex 的内建 MCP Client 只连接 Gateway，Gateway 再调用受策略治理的上游 MCP Server。
...
`dynamicTools` 当前是实验性 App Server API，必须在 `initialize.capabilities.experimentalApi=true` 后使用；若目标 Codex 版本不支持或协议契约测试失败，Codex Provider 应声明 `external_tools=false`，而不是降级为直连任何外部上游 MCP。

`dynamicTools` 只桥接 Knoa Capability Registry 的 Tool descriptors/calls，不承载或复制 MCP Resources、Prompts、Notifications 和 subscriptions；这些标准能力始终由 Knoa Standard MCP Host 与 Resource Task Bridge 处理。只有 Target Gateway 的 wire protocol 是标准 MCP。

Capability Gateway 不为 Codex 定义私有业务协议。是否实现取决于 Codex MCP Client 对审批和长调用语义的实际支持，当前 Jira Resource Task Bridge 不依赖 Gateway。
...
Provider 必须持久化 `threadId + turnId + itemId/callId + JSON-RPC request id + tool idempotency key` 桥接状态，并在执行外部动作前落库。断线时待决 approval/user-input 标记为 `provider_lost/expired`，未确认写操作进入 `outcome_unknown`；不能把 `thread/resume` 当作旧 Turn 或旧 server request 的恢复，也不得因 App Server/Gateway 重连自动重放。
...
Native Agent 和 Codex App Server 使用外部能力时 SHALL 经过 Knoa 的本地 capability、risk 和 approval policy。MCP Server 自述 SHALL 不能授权。
...
### REQ-MCP-011：Provider 无关

Resource Task 创建 SHALL 发生在 AgentRuntimeProvider 选择之前，Jira/PC Assistant MCP SHALL 不感知 Native 或 Codex。
...
本条是未来 Codex Provider Change 的兼容约束，不是当前 Jira Change 的验收门槛。后续 `CodexAppServerProvider` SHALL 使用完整 initialize 状态机和当前安装版本生成的协议 schema，将 Thread/Turn/Item、审批和中断映射到 Knoa 公共模型；使用隔离 Codex 配置并核验能力 inventory。Codex 使用 Jira/PC Assistant 等能力时 SHALL 通过 `dynamicTools -> Knoa Capability Registry` 或满足单一审批权威的 session-scoped 标准 MCP Gateway，生产模式 SHALL 不直接连接任何绕过 Knoa 本地策略的外部 MCP Server。
...
### 未来场景 E：Codex Provider

相同 Jira Resource Task 选择 Codex App Server Provider 后，Task/Conversation/Approval 语义保持不变；Codex 的外部工具调用仍回到 Knoa policy boundary。

### 未来场景 F：Codex 兼容性与安全降级

目标 Codex 版本通过其生成 schema 完成协议契约测试后才启用 Provider。若 `dynamicTools` 不受支持、App Server 断线或请求结果不确定，Knoa 保留 Task/Conversation 快照并将能力标记为不可用或进入 reconciliation；不得自动改为直连 Jira/PC Assistant MCP，也不得自动重放外部写操作。
...
| CODEX-CRIT-007 | dynamicTools 只桥接 Tools，不复制 MCP Resources/Prompts/Notifications |
| CODEX-CRIT-008 | Codex Provider 明确为未来独立 Change，不扩大当前 Jira 交付 |

## 17. 实现顺序

本次纵向实现交付第 1-6 项。第 7-8 项是基于已稳定 Port/Host 边界的独立后续 Change；本文只定义兼容和安全约束，不把 Codex App Server Provider 或 PC Assistant MCP 的全面迁移伪装成 Jira 示例的一部分。
...
7. 后续独立变化再实现 PC Assistant MCP 首个 workspace slice；
8. Codex App Server Provider 按既有 Runtime Provider 设计复用本能力平面。
