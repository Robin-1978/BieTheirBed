...
2. For images and scanned pages, use `attach_file` when needed so files are available to tools, then call `image_inspect` with focused questions:
...
9. Verify the report file exists and use `attach_file` when the user asked for a deliverable. Use `screenshot` only when capturing on-screen evidence adds value beyond the supplied files.
