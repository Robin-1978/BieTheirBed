"""Shared inbound conversation handling for IM channel adapters."""
...
@dataclass
class _PendingTextInteraction:
    interaction_id: str
    field_name: str


def _is_simple_text_interaction(
    kind: str,
    schema: dict[str, Any],
) -> tuple[bool, str]:
    """Return whether the interaction can be answered with a single text reply."""
    if kind != "user_input":
        return False, ""
...
class ChannelConversationMixin:
    ...
    def _init_conversation_runtime_state(self) -> None:
        ...
        self._active_session_presentations: dict[str, _ActiveTaskPresentation] = {}
        self._pending_confirmations: dict[str, _PendingConfirmation] = {}
        self._pending_confirmation_lock = threading.RLock()
        self._pending_text_interactions: dict[str, _PendingTextInteraction] = {}
    ...
    def _submit(self, coroutine: Any) -> None:
        ...
        def report_failure(done: Any) -> None:
            try:
                ...
                logger.exception("%s scheduled operation failed", self.name)
    ...
    async def _try_resolve_pending_interaction(
        self,
        principal_id: str,
        text: str,
    ) -> bool:
        pending = self._pending_text_interactions.get(principal_id)
        if pending is None:
            return False
        answer = text.strip()
        if not answer:
        ...
        try:
            await client.resolve_interaction(
                ...
                {pending.field_name: answer},
            )
        except CoreRequestError as exc:
            await asyncio.to_thread(
                self._send_text,
                principal_id,
                f"回复无效：{exc.message or exc.code}",
            )
            return True
        except Exception:
            logger.exception(
                "%s interaction resolution failed principal=%s",
                self.name,
                _principal_for_log(principal_id),
            )
            await asyncio.to_thread(
                self._send_text,
                principal_id,
            ...
            return True
        self._pending_text_interactions.pop(principal_id, None)
        await asyncio.to_thread(self._send_text, principal_id, "已收到。")
        return True

    @staticmethod
    def _interaction_text_prompt(
        ...
        field_title = str(field_schema.get("title") or "").strip()
        lines = []
        if title:
            lines.append(title)
        if description:
            lines.append(description)
        if field_title and field_title not in lines:
            lines.append(field_title)
        if not lines:
    ...
    async def _deliver_interaction_request(
        self,
        principal_id: str,
        client: CoreClient,
        event: TaskEvent,
        ...
        is_simple, field_name = _is_simple_text_interaction(
            payload.interaction_kind,
            payload.interaction_schema,
        )
        if is_simple:
            prompt = self._interaction_text_prompt(
                payload.interaction_display,
                payload.interaction_schema,
                field_name,
            )
            try:
                ...
                logger.warning(
                    "%s simple interaction prompt failed task_id=%s",
                    self.name,
                    event.task_id,
                    exc_info=True,
                )
            ...
            if not sent:
                return False
            self._pending_text_interactions[principal_id] = _PendingTextInteraction(
                interaction_id=payload.interaction_id,
                field_name=field_name,
            )
            return True
        return await self._deliver_complex_interaction_notice(
            principal_id,
            client,
            event,
        )

    async def _deliver_complex_interaction_notice(
        self,
        principal_id: str,
        client: CoreClient,
        event: TaskEvent,
        ...
        detail = str(
            event.payload.interaction_display.get("description", "")
        ).strip()
        ...
        try:
            ...
            logger.warning(
                "%s background interaction notification failed task_id=%s",
                self.name,
                event.task_id,
                exc_info=True,
            )
    ...
    async def _handle_text(
        ...
        try:
            ...
            if await self._try_resolve_pending_interaction(open_id, text):
                return
            ...
            try:
                ...
                logger.exception(
                    "%s Core Task failed principal=%s",
                    self.name,
    ...
    async def _run_text(self, open_id: str, text: str) -> None:
        ...
        if command == "/pause" and argument:
            try:
                result = await client.pause_task(argument, reason=f"{self.name} 手动暂停")
            except CoreRequestError as exc:
                if exc.code == "task_not_found":
        ...
        if command == "/resume" and argument:
            try:
                result = await client.resume_task(argument, reason=f"{self.name} 手动恢复")
            except CoreRequestError as exc:
                if exc.code == "task_not_found":
                    await asyncio.to_thread(self._send_text, open_id, "未找到该任务。")
                    return
