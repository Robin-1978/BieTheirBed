    ...
    <>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.card}>
          ...
          {gateway.status === "ready" ? <Metric label="P2P 状态" value={p2pStateLabel(gateway.p2pState)} /> : null}
          {gateway.p2pLastError ? <Text selectable style={styles.p2pError}>P2P 最近失败：{gateway.p2pLastError}</Text> : null}
...
function p2pStateLabel(state: "idle" | "connecting" | "ready" | "active" | "cooldown"): string {
  if (state === "connecting") return "正在建立 ICE 连接";
  if (state === "ready") return "DataChannel 已就绪，等待请求";
  if (state === "active") return "正在承载请求";
  if (state === "cooldown") return "失败后暂时使用 Relay";
  return "尚未尝试";
}
...
const styles = StyleSheet.create({
  container: { padding: 17, gap: 13, paddingBottom: 52 }, card: { padding: 16, gap: 10, borderRadius: 18, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line }, header: { flexDirection: "row", alignItems: "center", gap: 12 }, icon: { width: 48, height: 48, alignItems: "center", justifyContent: "center", borderRadius: 15, backgroundColor: colors.accentSoft }, flex: { flex: 1, minWidth: 0 }, title: { color: colors.ink, fontSize: 19, fontWeight: "800" }, online: { color: colors.accent, fontSize: 12, fontWeight: "700" }, offline: { color: colors.muted, fontSize: 12, fontWeight: "700" }, sectionTitle: { color: colors.ink, fontSize: 17, fontWeight: "800" }, metric: { flexDirection: "row", justifyContent: "space-between", gap: 12, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.line, paddingTop: 10 }, meta: { color: colors.muted, fontSize: 12, lineHeight: 18 }, transportDetail: { color: colors.muted, fontSize: 12, lineHeight: 18 }, p2pError: { color: colors.danger, fontSize: 11, lineHeight: 17, padding: 10, borderRadius: 10, backgroundColor: colors.background }, metricValue: { flex: 1, color: colors.ink, fontWeight: "700", textAlign: "right" }, compact: { fontFamily: "monospace", fontSize: 11 }, action: { minHeight: 62, flexDirection: "row", alignItems: "center", gap: 11, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.line }, actionTitle: { color: colors.ink, fontWeight: "800" }, remove: { minHeight: 48, alignItems: "center", justifyContent: "center" }, removeText: { color: colors.danger, fontWeight: "800" }, message: { color: colors.ink, backgroundColor: colors.accentSoft, borderRadius: 13, padding: 13 },
});
