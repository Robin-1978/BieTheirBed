...
ROLE_FILE="$CONFIG_ROOT/product-role"
...
if [ -f "$ROLE_FILE" ] && [ "$(cat "$ROLE_FILE")" != "$ROLE" ]; then
    printf 'Installed product role is %s; role changes require an explicit migration\n' "$(cat "$ROLE_FILE")" >&2
    exit 1
fi
...
if [ "$ROLE" = "hub" ] || [ "$ROLE" = "all" ]; then
    ...
    sed \
        ...
        -e "s|@HUB_ROOT@|$HUB_ROOT|g" \
        -e "s|@HUB_ENV@|$HUB_ENV|g" \
        "$CURRENT/install/knoa-hub.service" > /etc/systemd/system/knoa-hub.service
    chmod 644 /etc/systemd/system/knoa-hub.service
fi
...
if [ "$ROLE" = "node" ] || [ "$ROLE" = "all" ]; then
    ...
    sed \
        -e "s|@INSTALL_ROOT@|$INSTALL_ROOT|g" \
        -e "s|@NODE_ROOT@|$NODE_ROOT|g" \
        -e "s|@NODE_CONFIG@|$NODE_CONFIG|g" \
        -e "s|@WORKSPACE_ROOT@|$WORKSPACE_ROOT|g" \
        "$CURRENT/install/knoa-node.service" > /etc/systemd/system/knoa-node.service
    chmod 644 /etc/systemd/system/knoa-node.service
fi

chown -R knoa:knoa "$DATA_ROOT" "$CONFIG_ROOT"
