...
### 阶段一：P0 核心基建与成本突破（已完成落地 ✅）

- **目标**：彻底解决分词估算漂移与 Prompt Cache 穿透，消除窗口溢出 400 报错，大幅降低模型调用账单。
- **任务清单**：
  1. **插拔式真实 BPE 分词器适配**（已落地 ✅）：
     - 定义 `TokenEstimatorProtocol` 接口；
     - 实现基于 `tiktoken` 的高精度编码器（`cl100k_base`）；
     - 保留基于 CJK 的纯 Python 估算器作为无外部依赖时的优雅降级通道；
     - 专项测试验证代码、JSON、多语言下的估算误差控制在 1% 以内。
  2. **不可变前缀流（L0~L3）闭环**（已落地 ✅）：
     - 将单轮时间戳彻底锚定在 Turn 启动时刻（`turn_timestamp` 统一渲染传入）；
     - 杜绝单轮内跨迭代（iteration）刷新时间戳导致的公共子串破坏；
     - 优化上下文持久化组装逻辑，确保历史 Checkpoint 纯追加。

### 阶段二：P1 移动端体验跃升与长程韧性（已完成落地 ✅）

- **目标**：打造业界顶尖的移动端长任务交互体验，消除等待焦虑；保障外部工具长期运行零中断。
- **任务清单**：
  1. **移动端双轨流式渲染（Split-Stream UI）**（已落地 ✅）：
     - 拆解 `reasoning` 思考轨迹与 `content` 最终正文独立渲染；
     - 实现 `ThinkingCard` 呼吸态折叠卡片，实时反映思考状态与微交互指示器；
     - 工具调用状态升级为独立轻量圆角胶囊组件（28dp）。
  2. **工件交互卡片化与一键导出**（已落地 ✅）：
     - 移动端原生卡片识别 `artifact_id`，提供“快速预览抽屉”、“ANSI 高亮全屏查看”及“一键分享/另存为文件”。
  3. **stdio MCP 进程守护看门狗（Auto-Respawn）**（已落地 ✅）：
     - 在 `src/knoa_platform/extensions/mcp.py` 引入自动健康探测；
     - 识别异常崩溃（Connection Closed / BrokenPipe / code=-32000）；
     - 自动触发热重启与通道重连，无需人工干预。
  4. **系统级任务通知推送（APNs / FCM）与深链接**（已落地 ✅）：
     - 任务挂起等待审批（Waiting Approval）或完成时在后台主动触发本地系统通知；
     - 点击通知直接通过 `taskId` / `executionId` 进行深度链接唤醒并跳转对应任务页面。

### 阶段三：P2 分布式网关与并发加固（已完成落地 ✅）

- **目标**：提高弱网下的协议容错性，彻底解决并发写库锁争用与分布式边缘节点韧性。
- **任务清单**：
  1. **移动端 Delta SSE 乱序与重连补偿**（已落地 ✅）：
     - 在 `chatTurns.ts` 中引入 `revision` 序列校验器；
     - 严格过滤重复 revision，识别到跨步空洞（gap）时立即通知 Watcher 重新拉取基线 Snapshot。
  2. **SQLite 高频写缓冲与读写锁分离**（已落地 ✅）：
     - 默认启用 `PRAGMA busy_timeout = 5000`（5s 锁排队重试）；
     - 默认启用 `PRAGMA synchronous = NORMAL`，减少 WAL 模式下的磁盘 sync 阻塞；
     - 将 TaskExecutor 中高频流式 Trace 写库间隔由 0.5s 平滑延长至 1.5s，大幅减轻 WAL 锁争用。
  3. **工件沙箱与跨租户路径隔离加固**（已落地 ✅）：
     - 移除 64 位十六进制会话 ID 特权通道，统一强制执行 SHA-256 会话键哈希；
     - 严格杜绝目录穿越与跨租户越权访问风险。
