...
### AR-017: Context budgeting is advisory, not a hard request limit — high

`truncate_messages()` stops removing history once two turns remain and cannot
reduce an oversized current turn or pinned system/session/memory segments. Tool
schema tokens are also outside its calculation. A request can therefore exceed
`context_window_budget` while only emitting a warning. Replace the single soft
budget with a `ContextBudgetPlanner` that accounts for system policy, selected
tool schemas, memory, transcript, observations, and reserved completion tokens;
it must fail or degrade deterministically when mandatory segments do not fit.

### AR-018: External channels can expose model reasoning — high

The Feishu adapter collects `stream_think_delta` and renders it in the response
card. Channel users should receive progress state and the final answer, not raw
model reasoning. Thinking events may remain available to explicitly enabled
local diagnostics, but production channels should discard them or replace them
with bounded status events generated by the orchestrator.
...
### AR-021: `/compact` currently means clear — medium

Both local UI implementations clear the conversation, and the service command
drops the session, while reporting that context was compacted. Route all clients
through one Agent command API and call `ConversationManager.compress()` for
compaction; reserve session deletion for `/clear`.
...
### AR-023: In-memory transcript retention is unbounded per live session — medium

`ConversationManager(max_messages=...)` accepts but intentionally ignores its
limit, while request assembly selects only recent turns. Long-lived sessions can
therefore grow in memory even though old messages no longer reach the model.
Replace the misleading parameter with an explicit retention policy that keeps a
bounded recent transcript plus an optional summary/reference archive.
...
| P1 | Context budget can be exceeded by mandatory/current segments and tool schemas | Truncation only drops older turns and does not budget the full provider request | Typed segment budget planner with completion reserve and deterministic degradation | open |
| P1 | Feishu can publish raw model thinking | Channel renders `stream_think_delta` as response-card content | Channel-safe progress events; discard raw reasoning outside explicit local diagnostics | open |
...
| P2 | `/compact` clears the session | Three client implementations duplicate command semantics | One Agent command contract invoking real conversation compression | open |
| P2 | Live-session transcript grows without a retention bound | `max_messages` is non-functional and only request assembly is bounded | Explicit transcript retention and summary/archive policy | open |
